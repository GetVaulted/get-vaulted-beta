/**
 * Loads SHIPPO_API_TOKEN from Netlify (never prints it), then runs the complete
 * failed-label clawback exposure scan + repair manifest generator.
 *
 * Usage: npx tsx scripts/run-failed-label-clawback-exposure-with-netlify-shippo.ts
 */
...
async function loadWorkingShippoFromNetlify(): Promise<{ token: string; context: string } | null> {
  for (const context of contexts) {
    ...
    try {
      ...
      if (probe.status !== 401 && probe.status !== 403) return { token, context };
    } catch {
      // next
    }
  }
...
async function main() {
  ...
  if (!loaded) {
    console.error(JSON.stringify({ error: "NO_WORKING_SHIPPO_TOKEN_IN_NETLIFY_CONTEXTS" }));
    process.exit(1);
  }
  ...
  const run = spawnSync(
    "npx",
    ["tsx", "scripts/scan-failed-label-clawback-exposure.ts", ...extra],
    { cwd: webRoot, encoding: "utf8", shell: true, env: process.env, stdio: "inherit" },
  );
