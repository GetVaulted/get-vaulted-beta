/**
 * Home banner promo image uploads (admin-only): Supabase Storage when configured, otherwise
 * `public/uploads/banners/` (local dev). Response is always `{ url }`. Reuses the same
 * Supabase Storage bucket/client as listing images — see `@/lib/supabase-listing-storage`.
 */
...
export async function POST(req: Request) {
  ...
  const name = `banners/${randomUUID()}.${ext}`;
