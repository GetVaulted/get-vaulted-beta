...
/**
 * OBS / advanced stream setup for external (non-webcam) streamers: RTMPS server + stream key,
 * provision/rotate, and playback status. The in-browser webcam broadcast is handled at the
 * console level via `useHostWebcamBroadcast` and the stage Go Live / Stop Stream pill.
 */
export function HostStreamSetupCard({
  ...
  return (
    <section className={`rounded-xl border border-white/[0.08] bg-black/30 ${compact ? "p-3" : "p-4"}`}>
      ...
      <details className="group mt-0 rounded-lg border border-white/10 bg-zinc-950/40" open>
        <summary className="cursor-pointer list-none px-3 py-2.5 text-xs font-semibold text-zinc-300 marker:content-none [&::-webkit-details-marker]:hidden">
          <span className="inline-flex items-center gap-2">Advanced / OBS</span>
        </summary>
