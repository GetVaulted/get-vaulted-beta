...
import { formatDivisionReelAbbr, NFL_DIVISION_COLORS } from "@/lib/nfl-team-colors";
...
export type VaultRevealSpinPayload = {
  ...
  durationMs: number;
  referenceId?: string;
  /** Open watch-enter givvy vs buyers purchase-entry givvy. */
  giveawayKind?: VaultRevealGiveawayKind;
  /** Resolved giveaway winner — lets clients show personal win copy. */
  winnerUserId?: string;
...
/** Givvy draw chrome — matches mobile `givvyUi.ts`. */
...
export { formatDivisionReelAbbr } from "@/lib/nfl-team-colors";
...
export function isDivisionRevealSpin(spin: Pick<VaultRevealSpinPayload, "labels">): boolean {
  return spin.labels.some((label) => {
    ...
    return NFL_DIVISIONS_PRESET.some((division) => formatDivisionReelAbbr(division.label) === trimmed);
  });
}
...
/** Personal banner when the signed-in viewer won this givvy draw. */
